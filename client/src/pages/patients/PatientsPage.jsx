...
const PatientsPage = () => {
  const { items, loading, error, setParams, createItem, updateItem, deleteItem } = useResource('patients');
  ...
  const handleDelete = async (record) => {
    if (!window.confirm(`Delete patient ${record.firstName} ${record.lastName}?`)) {
      return;
    }
    try {
      await deleteItem(record.id);
    } catch (err) {
      const message = err.response?.data?.message || 'Unable to delete patient.';
      alert(message);
    }
  };
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Unable to save patient.');
    }
  };
  ...
  const columns = useMemo(
    () => [
      ...
      { key: 'gender', label: 'Gender', render: (value) => value?.charAt(0).toUpperCase() + value?.slice(1) },
      {
        key: 'dateOfBirth',
        label: 'DOB',
    ...
    <div className="space-y-6">
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Manage patient records, contact details, and assignments.</p>
        </div>
        <button
          type="button"
      ...
      {isModalOpen ? (
        <EntityFormModal
          isOpen={isModalOpen}
          title={editingRecord ? 'Edit Patient' : 'Add Patient'}
          fields={modalFields}
          initialValues={initialValues}
          onSubmit={handleSubmit}
          onClose={handleModalClose}
          submitLabel={editingRecord ? 'Update Patient' : 'Create Patient'}
        />
      ) : null}
